...
import (
	...
	"github.com/zxylon/olaf/internal/pkg/helper"
)
...
func init() {
	CmdRun.Flags().StringVarP(&excludeDir, "excludeDir", "", excludeDir, `eg: olaf run --excludeDir="tmp,vendor,.git,.idea"`)
	CmdRun.Flags().StringVarP(&includeExt, "includeExt", "", includeExt, `eg: olaf run --includeExt="go,tpl,tmpl,html,yaml,yml,toml,ini,json"`)
	if excludeDir == "" {
...
var CmdRun = &cobra.Command{
	Use:     "run",
	Short:   "olaf run [main.go path]",
	Long:    "olaf run [main.go path]",
	Example: "olaf run cmd/server",
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("\033[35mOlaf run %s.\033[0m\n", dir)
